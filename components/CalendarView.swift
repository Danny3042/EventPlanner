...
public struct CalendarView: UIViewRepresentable {
    ...
    public func events(for dateComponents: DateComponents) -> [Event] {
        let calendar = Calendar.current
        return events.filter { event in
            let eventDateComponents = calendar.dateComponents([.year, .month, .day], from: event.date)
            return eventDateComponents == dateComponents
        }
    }
...
extension CalendarView.Coordinator: UICalendarViewDelegate {
    public func calendarView(_ calendarView: UICalendarView, decorationFor dateComponents: DateComponents) -> UICalendarView.Decoration? {
        if #unavailable(iOS 16.2) {
            ...
            if parent.events(for: dateComponents).isEmpty {
                return nil
            } else {
                return UICalendarView.Decoration.image(UIImage(systemName: "star"),color: UIColor.red, size: .medium)
            }
        }
